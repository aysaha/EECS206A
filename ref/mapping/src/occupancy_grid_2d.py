...
class OccupancyGrid2d(object):
    ...
    def SensorCallback(self, msg):
        ...
        for idx, r in enumerate(msg.ranges):
            ...
            # TODO!
            theta = yaw + msg.angle_min + (msg.angle_increment*idx)
            # print(r)
            ...
            # TODO!
            obj_x = sensor_x + r*np.cos(theta)
            obj_y = sensor_y + r*np.sin(theta)

            m = (obj_y - sensor_y) / (obj_x - sensor_x)

            prevPoint = self.PointToVoxel(sensor_x, sensor_y)
            samples = 250
            for i in range(samples):
                x = sensor_x + i * (obj_x - sensor_x) / samples
                y = sensor_y + i * (obj_y - sensor_y) / samples
                
                newPoint = self.PointToVoxel(x, y)
                if (newPoint != prevPoint):
                    self._map[newPoint] += self._free_update
                    if (self._map[newPoint] < self._free_threshold):
                        self._map[newPoint] = self._free_threshold
                    prevPoint = newPoint

            lastPoint = self.PointToVoxel(obj_x, obj_y)
            
            if (prevPoint == lastPoint):
                self._map[lastPoint] -= self._free_update
            self._map[lastPoint] += self._occupied_update
            if (self._map[lastPoint] > self._occupied_threshold):
                self._map[lastPoint] = self._occupied_threshold
